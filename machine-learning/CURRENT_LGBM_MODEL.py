...
for parking_lot_to_predict in og_parking_lots:
    ...
    def add_lags(df):
        ...
        if short_term_4_hours:
            for lag_hours in range(4, 25):  # lags for 1 to 24 hours
                df[f'lag_{lag_hours}_hours'] = (df.index - pd.Timedelta(f'{lag_hours} hours')).map(target_map['Occupancy'])
                #df[f'lag_{lag_hours}_hours'] = df['Occupancy'].shift(lag_hours)

        

        if short_term_1_week:
            for lag_days in range(1, 20):  # lags for 1 week and more
                df[f'lag_{lag_days*7}_days_Occupancy'] = (df.index - pd.Timedelta(f'{lag_days*7} days')).map(target_map['Occupancy'])
        ...
        if short_term_1_day:
            ...
            for lag_days in range(1, 7):  # lags for 1 to 6 days
                df[f'lag_{lag_days}_days'] = (df.index - pd.Timedelta(f'{lag_days} days')).map(target_map['Occupancy'])
    ...
    plt.savefig(f'lgbm_charts/{parking_lot_to_predict}_actual_vs_pred.png')
    ...
    error_data = {
        ...
        parking_lot_to_predict: [rmse, mae, mse, r2]
    }
    ...
    csv_file_path = "parking_lot_metrics.csv"
    ...
    plt.savefig(f'lgbm_charts/{parking_lot_to_predict}_feature_importance.png')
