...
for time_duration in range(1, 3):
    ...
    for parking_lot_to_predict in ["North"]:
        parking_lots = og_parking_lots.copy()
        ...
        actual_vs_pred_df.to_csv("actual_vs_pred_initial.csv")
        ...
        actual_vs_pred_df_loaded = pd.DataFrame({'Actual': y_test, 'Predicted': y_pred_loaded}, index=test.index)
        actual_vs_pred_df_loaded_fixed = actual_vs_pred_df_loaded.copy()
        actual_vs_pred_df_loaded_fixed.reset_index(inplace=True)

        # Export to CSV
        actual_vs_pred_df_loaded_fixed.to_csv('actual_vs_pred_loaded.csv', index=False)

        print(X_test)
        print(X_test.columns)

        # Include the index (date) as a column
        X_test.reset_index(inplace=True)
        # Assuming X_test is already defined
        X_test_df = pd.DataFrame(X_test, columns=['date', 'term_date', 'is_holiday', 'day_of_week', 'month', 'day_of_year',
                                                'week_of_year', 'hour', 'weighted_day_of_year', 'year',
                                                'is_month_start', 'is_month_end', 'sin_hour', 'cos_hour', 'sin_day',
                                                'cos_day', 'sin_week', 'cos_week', 'sin_month', 'cos_month', 'is_day',
        ...
        # Check consistency
        consistency_check = np.allclose(y_pred, y_pred_loaded)
